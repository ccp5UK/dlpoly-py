...
from .utility import DLPData, strip_comments
...
class NewControl(DLPData):
    ...
    def _(self, data):
        ...
        """
        for line in strip_comments(data, comment_char="#!"):
            if not line:
                continue
            key, *args = line.split()
            ...
            if key.startswith("ewald"):
                ...
                print(
                    f"Warning {key} used in control, should be {corrected_key}"
                    " (applying correction)",
                    flush=True,
                )
                key = corrected_key

            self[key] = [stripped_arg for arg in args if (stripped_arg := arg.strip("[]"))]
    ...
    def write(self, filename: PathLike = "new_control"):
        ...
        with open(filename, "w", encoding="utf-8") as out_file:
            ...
            for key, vals in self.items():
                if key in {"title", "filename", "io_file_control"} or (
    ...
    def __iter__(self) -> Iterator[str]:
        ...
        return (
            ...
            if (not key.startswith("_") and key not in ("source") and vals is not None)
        )
...
"""
        out += "\n".join(
            f"{key}: {self._format_val(vals, key)}" for key, vals in self.items() if key != "title"
        )
        return out
...
def is_new_control(filename: PathLike) -> bool:
    ...
    """
    with open(filename, "r", encoding="utf-8") as in_file:
        for line in strip_comments(in_file, "#!"):
            if not line:
                continue
