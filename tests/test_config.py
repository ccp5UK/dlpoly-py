...
class ConfigTest(unittest.TestCase):
    ...
    def setUp(self):
        self.config = dlp.DLPoly(config="CONFIG").config
    ...
    def test_config_cell(self):
        self.assertListEqual(list(self.config.cell[0,:]), [112.3843994140,0.0000000000,0.0000000000],
                         'incorrect cell a')
        self.assertListEqual(list(self.config.cell[1,:]), [0.0, 94.3219299316, 0.0],
                         'incorrect cell b')
        self.assertListEqual(list(self.config.cell[2,:]), [0.0,0.0,94.2717666626],
                         'incorrect cell c')
    ...
    def test_config_atom(self):
        ...
        self.assertEqual(self.config.atoms[100].index, 101,
                         'incorrect index')
        self.assertListEqual(list(self.config.atoms[100].pos),[-46.97998686,8.725645151,-46.70341089],
                               'incorrect positions')
        self.assertListEqual(list(self.config.atoms[100].vel),[6.067870998,-6.202633836,-4.581153286],
                               'incorrect veloctities')
        self.assertListEqual(list(self.config.atoms[100].forces),[-114729.0839,191994.0141,-110473.8008],
                               'incorrect forces')
